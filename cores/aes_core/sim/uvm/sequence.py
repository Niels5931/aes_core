...
class AesSequence(uvm_sequence):
    async def body(self):
        for _ in range(10):  # Generate 10 transactions
            aes_tr = AesTransaction()
            await self.start_item(aes_tr)
            aes_tr.randomize_operands()
            await self.finish_item(aes_tr)

    #def _simulate_output(self, txn):
    ...
    #    if self.corenum == 0:
    #        # test initial round
    #        expected = txn.data_in ^ txn.key_in
    #    elif self.corenum == 9:
    #        #subbytes
    #        sub = [sbox[(txn.data_in >> (i * 8)) & 0xFF] for i in range(16)]
    ...
    #        expected = int(shiftrow, 2) ^ txn.key_in
    #    else:
    #        # test second round
    #        pass
    #    return expected